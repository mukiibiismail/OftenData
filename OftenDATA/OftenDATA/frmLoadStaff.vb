...
Public Class frmLoadStaff
    ...
    Private Sub LoadStaff()
        Try
            sqL = "SELECT StaffID, Lastname & ', ' & Firstname & ' ' & mi as StaffName, Positions FROM STAFF Order by Lastname"
            ConnDB()
    ...
    Private Sub SearchStaff()
        Try
            sqL = "SELECT StaffID, Lastname & ', ' & Firstname & ' ' & mi as StaffName, Positions FROM STAFF WHERE Lastname LIKE '" & TextBox1.Text & "%' Order by Lastname"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
    ...
    Private Sub dgw_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgw.CellDoubleClick
        ...
        If frmUsers.uSearch = True Then
            ...
            Me.Close()
        End If
        'If frmRights.uSearch = True Then
        '    frmRights.txtStaffID.Text = dgw.CurrentRow.Cells(0).Value
        '    frmRights.txtUser.Text = dgw.CurrentRow.Cells(1).Value
        '    frmRights.uSearch = False
        '    Me.Close()
        'End If
    End Sub
